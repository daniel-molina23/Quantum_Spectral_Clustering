#### Most of this code from taken from 
"""
Most of this code was taken from: https://github.com/SRI-International/QC-App-Oriented-Benchmarks/blob/master/phase-estimation/qiskit/pe_benchmark.py
...
"""
Phase Estimation Program - Qiskit
"""
...
from quantum_fourier_transform import inv_qft_gate
...
def QPE(num_qubits=3, num_circuits=3, num_shots=100,
        backend_id='qasm_simulator', provider_backend=None,
        hub="ibm-q", group="open", project="main", exec_options=None):

    print("Quantum Phase Estimation Program - Qiskit")

    num_state_qubits = 1 # default, not exposed to users, cannot be changed in current implementation

    num_qubits = max(num_state_qubits + 2, num_qubits) # minimum of 3 qubits
    ...
    # ONLY DO A SINGLE EXECUTION OF QUANTUM PHASE ESTIMATION
    # reset random seed
    np.random.seed(0)
    ...
    num_circuits = min(2 ** (num_counting_qubits), num_circuits)
    
    print(f"************\nExecuting [{num_circuits}] circuits with num_qubits = {num_qubits}")
    
    # determine range of secret strings to loop over
    if 2**(num_counting_qubits) <= num_circuits:
