# This was taken from: https://github.com/QuantumComputingLab/fable/blob/main/py/fable/fable.py regarding the paper https://arxiv.org/pdf/2205.00081.pdf

#!/usr/bin/env python
# -*- coding: utf-8 -*-
import numpy as np
from qiskit import QuantumCircuit
from _common._util import compressed_uniform_rotation, sfwht, gray_permutation
...
def fable(a, epsilon):
    ...
    circ, alpha = fable_with_existing_circuit(a, epsilon, circ=circ)
...
def fable_with_existing_circuit(a, epsilon, circ, offset_qubit=0):
    '''FABLE - Fast Approximate BLock Encodings.
    ...
    for i in range(logn):
        circ.h(offset_qubit+i+1)

    # matrix oracle
    q_range = [offset_qubit + i for i in range(2*logn + 1)]
    circ = circ.compose(OA, qubits=q_range) ################ OHHHHH SHIIIIIT
    
    # swap register
    for i in range(logn):
        circ.swap(offset_qubit+i+1,  offset_qubit+i+logn+1)
